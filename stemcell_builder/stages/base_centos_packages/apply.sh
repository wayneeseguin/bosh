...
case "${stemcell_operating_system_version}" in
  ...
  (7|7.*) 
    ...
    # note about dip group: it has been removed in CentOS 7, but the 
    # os-independent stuff elsewhere in stemcell builder assumes the group 
    # exists. So we create it here.
...
# The CentOS 6.5 script upgraded upstart first,
# "to prevent it from messing up our stubs and starting daemons anyway"
...
packages=(
openssl-devel lsof strace bind9-host dnsutils tcpdump iputils-arping curl wget 
libcurl3 libcurl3-dev bison readline-devel libxml2 libxml2-devel libxslt 
libxslt-devel dhclient zip unzip nfs-common flex psmisc apparmor-utils iptables 
sysstat rsync openssh-server traceroute libncurses5-dev quota libaio1 gdb 
libcap2-bin libcap-devel bzip2-devel cmake sudo libuuid-devel parted 
NetworkManager e2fsprogs
)
